...
export interface HyperspeedEffectOptions {
  onSpeedUp?: (event: MouseEvent | TouchEvent) => void;
  onSlowDown?: (event: MouseEvent | TouchEvent) => void;
  ...
  useEffect(() => {
    ...
    class App {
      ...
      constructor(container: HTMLDivElement, options: HyperspeedEffectOptions) {
        ...
        this.camera = new THREE.PerspectiveCamera(options.fov ?? 90, initW / initH, 0.1, 10000);
        ...
        const bg = options.colors?.background ?? 0x000000;
        const fog = new THREE.Fog(bg, length * 0.2, length * 500);
        this.scene.fog = fog;
        this.fogUniforms = {
      ...
      init() {
        ...
        this.leftCarLights.mesh.position.setX(-(options.roadWidth ?? 10) / 2 - (options.islandWidth ?? 2) / 2);
        this.rightCarLights.init();
        this.rightCarLights.mesh.position.setX((options.roadWidth ?? 10) / 2 + (options.islandWidth ?? 2) / 2);
        this.leftSticks.init();
        this.leftSticks.mesh.position.setX(-((options.roadWidth ?? 10) + (options.islandWidth ?? 2) / 2));
      ...
      onTouchStart(ev: TouchEvent) {
        if (this.options.onSpeedUp) this.options.onSpeedUp(ev as unknown as TouchEvent);
        this.fovTarget = this.options.fovSpeedUp ?? 150;
        this.speedUpTarget = this.options.speedUp ?? 2;
      }

      onTouchEnd(ev: TouchEvent) {
        if (this.options.onSlowDown) this.options.onSlowDown(ev as unknown as TouchEvent);
        this.fovTarget = this.options.fov ?? 90;
        this.speedUpTarget = 0;
      }
      ...
      update(delta: number) {
        ...
        if (this.options.distortion && (this.options.distortion as Distortion).getJS) {
    ...
    class CarLights {
      ...
      constructor(webgl: App, options: HyperspeedEffectOptions, colors: number[] | number | undefined, speed: number[] | undefined, fade: THREE.Vector2) {
        this.webgl = webgl;
        this.options = options;
        this.colors = colors;
        this.speed = speed;
      ...
      init() {
        ...
        let colors: number[] | number | THREE.Color | THREE.Color[] | undefined = this.colors;
        ...
        for (let i = 0; i < (options.lightPairsPerRoadWay ?? 40); i++) {
          ...
          const speed = random(this.speed ?? [60, 80]);
          ...
          const color = pickRandom<THREE.Color | number[]>(colors as THREE.Color[]);
          aColor.push(color instanceof THREE.Color ? color.r : 1, color instanceof THREE.Color ? color.g : 1, color instanceof THREE.Color ? color.b : 1);
          aColor.push(color instanceof THREE.Color ? color.r : 1, color instanceof THREE.Color ? color.g : 1, color instanceof THREE.Color ? color.b : 1);
        }
        ...
        const material = new THREE.ShaderMaterial({
          ...
          uniforms: Object.assign(
            {
              uTime: { value: 0 },
              uTravelLength: { value: options.length ?? 400 },
              uFade: { value: this.fade },
            },
            this.webgl.fogUniforms,
            (options.distortion as Distortion).uniforms,
          ),
        });
    ...
    class LightsSticks {
      ...
      init() {
        ...
        let colors: THREE.Color | THREE.Color[] | number | undefined = options.colors?.sticks;
        if (Array.isArray(colors)) {
          colors = colors.map((c) => new THREE.Color(c));
        } else if (typeof colors === 'number') {
          colors = new THREE.Color(colors);
        ...
        for (let i = 0; i < totalSticks; i++) {
          ...
          const color = pickRandom<THREE.Color>(colors as THREE.Color[]);
    ...
    class Road {
      ...
      createPlane(side: number, width: number, isRoad: boolean) {
        ...
        const geometry = new THREE.PlaneGeometry(
          isRoad ? (options.roadWidth ?? 10) : (options.islandWidth ?? 2),
          options.length ?? 400,
          20,
          segments,
        );
        const uniforms: Record<string, { value: THREE.Color | number }> = {
          uTravelLength: { value: options.length ?? 400 },
          uColor: {
            value: new THREE.Color(
              isRoad ? (options.colors?.roadColor ?? 0x080808) : (options.colors?.islandColor ?? 0x0a0a0a),
            ),
          },
          uTime: this.uTime,
        };
        ...
        if (isRoad) {
          Object.assign(uniforms, {
            uLanes: { value: options.lanesPerRoad ?? 4 },
            uBrokenLinesColor: { value: new THREE.Color(options.colors?.brokenLines ?? 0xffffff) },
            uShoulderLinesColor: { value: new THREE.Color(options.colors?.shoulderLines ?? 0xffffff) },
            uShoulderLinesWidthPercentage: { value: options.shoulderLinesWidthPercentage ?? 0.05 },
            uBrokenLinesLengthPercentage: { value: options.brokenLinesLengthPercentage ?? 0.5 },
            uBrokenLinesWidthPercentage: { value: options.brokenLinesWidthPercentage ?? 0.1 },
          });
        }
        ...
        const material = new THREE.ShaderMaterial({
          ...
          side: THREE.DoubleSide,
          uniforms: Object.assign(uniforms as never, this.webgl.fogUniforms, (options.distortion as Distortion).uniforms),
        });
      ...
      init() {
        this.leftRoadWay = this.createPlane(-1, this.options.roadWidth ?? 10, true);
        this.rightRoadWay = this.createPlane(1, this.options.roadWidth ?? 10, true);
        this.island = this.createPlane(0, this.options.islandWidth ?? 2, false);
      }
    ...
    const options: HyperspeedEffectOptions = {

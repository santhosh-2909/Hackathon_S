...
export const PROBLEM_STATEMENTS: readonly ProblemStatement[] = [
  ...
  /* ---------------------------------------------------------------------------
     11 — Student Innovation Idea.
     Appended after the ten official statements and deliberately separate from
     them: no portal code, no ministry, and `origin: 'student-innovation'` so
     every surface marks it as a student idea rather than an official SIH
     problem statement. The ten entries above are unchanged.
     ------------------------------------------------------------------------ */
  {
    slug: 'hostel-mess-food-waste',
    origin: 'student-innovation',
    code: null,
    year: 2026,
    title: 'Hostel mess food waste from an unpredictable headcount',
    summary:
      'Mess managers cook for full strength while actual attendance swings by a third on weekends and exam days — so cooked food is thrown out nightly.',
    domain: 'student-innovation',
    organization: 'Student innovation idea — no sponsoring organisation',
    theme: 'Student Innovation (unofficial)',
    category: 'Software',
    difficulty: 'medium',
    dataset: 'self-sourced',
    datasetNote:
      'No dataset exists for this. A team collects its own: two to three weeks of mess attendance counts and end-of-service waste weights from a single hostel, plus the academic calendar. That log is the project — a forecast trained on a fortnight of real counts is more convincing to a judge than a model trained on someone else’s numbers.',
    background:
      'A hostel mess cooks to sanctioned strength because that is the only number it has. Real attendance moves constantly: students go home at weekends, skip breakfast in exam weeks, and leave early for placement drives. Nobody tells the kitchen, so the kitchen assumes everyone. The gap is absorbed as cooked food scraped into bins at the end of every service — paid for out of the same mess fee the students complain about, and invisible because nobody weighs it.',
    statement:
      'Hostel mess managers struggle to decide how much to cook when daily attendance swings by thirty to forty per cent, because students never signal whether they will eat, which leads to large quantities of cooked food discarded at the end of every service.',
    expectedSolution:
      'A meal-confirmation loop with a forecast behind it. Students opt out of individual meals in one tap from a phone, up to a cutoff before cooking starts. The mess sees a predicted headcount that blends those confirmations with a short-horizon forecast learnt from historical attendance and the academic calendar, broken down into per-dish quantities. Surplus that still occurs is logged by weight and offered to a registered NGO through a pickup request, so the waste that is not prevented is at least redirected rather than binned.',
    whyItFits:
      'The pain is observable on any campus in a single evening with a weighing scale, and the persona is one person you can actually interview — the mess manager. The vertical slice is honest and small: opt-out screen, cutoff job, headcount view. The forecast can start as a weekday-average baseline and only then be replaced by a model, which keeps a demo working even if the modelling stalls.',
    keyFeatures: [
      'One-tap meal opt-out with a hard cutoff before cooking begins',
      'Predicted headcount blending confirmations with a calendar-aware forecast',
      'Per-dish quantity guidance for the kitchen, not just a single total',
      'Surplus logged by weight, with an NGO pickup request',
      'Weekly waste trend in kilograms and rupees, visible to students and the warden',
    ],
    expectedImpact:
      'The measurable outcome is kilograms of cooked food discarded per service, before and after. A mess that can see tomorrow’s headcount within a reasonable margin can cut over-preparation directly, which lowers both waste and the per-student mess bill; whatever surplus remains reaches a food bank instead of a bin. Publishing the weekly figure is itself part of the intervention — waste that is counted stops being invisible.',
    innovationScope:
      'The novelty is not the forecast, which is ordinary; it is closing the loop between a behavioural nudge, a kitchen decision and a redistribution handoff in one system that a single hostel can adopt without procurement. It extends naturally to multiple hostels on one campus, then to raw-material purchasing rather than daily cooking, and the same opt-out-plus-forecast shape transfers to campus transport and lab slots.',
    deliverables: [
      'Student PWA with opt-out and cutoff handling',
      'Mess dashboard: predicted headcount, per-dish quantities, surplus log',
      'Forecast service with a documented baseline to compare against',
      'A fortnight of real attendance and waste measurements from one mess',
      'Before/after waste report and a short pilot write-up',
    ],
    stack: ['Next.js PWA', 'Supabase', 'Postgres', 'Python + scikit-learn', 'Vercel'],
    image: '/imagery/hero-3.avif',
    sliceHours: 6,
  },
] as const;
